...
import { Users, ShieldCheck, HeartHandshake, Globe2 } from "lucide-react";
...
function TeamSection({
  ...
  return <section className="space-y-6"><div className="flex items-center gap-2 border-b border-slate-200 pb-3"><Icon className="w-5 h-5 text-teal-600" /><h2 className="text-xl font-extrabold text-slate-900">{title}</h2></div><div className={`grid grid-cols-1 ${gridClass} gap-6`}>{members.map((member) => <div key={member.id} className="bg-white rounded-2xl border border-slate-200/80 p-6 space-y-3 shadow-xs"><span className={`text-[10px] font-bold px-2 py-0.5 rounded-md uppercase tracking-wider block w-fit ${badgeClass}`}>{member.roleTitleFr}</span><h3 className="font-bold text-slate-900 text-sm">{member.name}</h3><p className="text-xs text-slate-500 leading-relaxed">{member.bioFr}</p></div>)}</div></section>;
}
